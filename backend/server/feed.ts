...
export class Feed {
  ...
  /** Set by the settings controller; samples carry queen: true for her. */
  queenUid = '';

  constructor(registry: Registry, swarm: Swarm, global: Global, vision: VisionIn, mix: Mix) {
    ...
    registry.on('sample', (s) => this.broadcast({ type: 'sample', slot: s.slot, uid: s.uid, t: s.t, acc: s.acc, gyro: s.gyro, rel: s.rel, activity: s.activity, idle: s.idle, turn: s.turn, queen: this.queenUid !== '' && s.uid === this.queenUid }, true));
    ...
    mix.on((m) => this.broadcast({ type: 'mix', ...m }, true));
  }
